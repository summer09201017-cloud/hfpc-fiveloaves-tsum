...
function collect(list){
  ...
  fed = Math.min(curTarget, fed + people);
  chainCount++;
  hintT = 0; hintGroup = null;
  chordCollect(n);
  for (var i=0;i<n;i++){
...
function drawHUD(){
  ...
  ctx.fillText('已吃飽 ' + Math.round(shownFed) + ' / ' + curTarget + ' 人', W/2, 40);
  // 返回大廳
...
function startGame(){
  try{ level = Math.max(1, parseInt(localStorage.getItem('l2f-lvl-'+modeKey))||1); }catch(e){ level = 1; }
...
function winTap(p){
  for (var i=0;i<winBtns.length;i++){
    var b = winBtns[i];
    if (p.x>b.x && p.x<b.x+b.w && p.y>b.y && p.y<b.y+b.h){
      if (b.act==='next'){ try{ localStorage.setItem('l2f-lvl-'+modeKey, ''+(level+1)); }catch(e){} startGame(); return; }
