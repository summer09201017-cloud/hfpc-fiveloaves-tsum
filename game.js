...
var BURST = { icon:'🧺', name:'分餅籃', banner:'🧺 擘開遞下去!附近的都分到了!' };
...
function maxLevel(){ try{ return Math.max(1, parseInt(localStorage.getItem('l2f-lvl-'+modeKey))||1); }catch(e){ return 1; } }
function loadStars(){ try{ stars = JSON.parse(localStorage.getItem('l2f-stars-'+modeKey)||'{}')||{}; }catch(e){ stars = {}; } }
function saveStars(lv, st){
  if ((stars[lv]|0) >= st) return;
  stars[lv] = st;
  try{ localStorage.setItem('l2f-stars-'+modeKey, JSON.stringify(stars)); }catch(e){}
}
...
function sprintEnd(){
  ...
  speak('win');
  if (!doneSent){ doneSent = true; if (window.__ping) window.__ping('fiveloaves-tsum-done', Math.round((Date.now()-startTime)/1000)); }
}
...
function collect(list){
  ...
  else if (goldN > 0){ banner = { text:'✨ 金色雙倍!多餵飽 ' + (goldN*mult) + ' 人', t:1.5 }; }
...
function drawHUD(){
  ...
  if (sprint){ ctx.fillText('⏱ ' + Math.ceil(sprintT) + ' 秒・餵飽 ' + Math.round(shownFed) + ' 人', W/2, 40); }
  else { ctx.fillText('已吃飽 ' + Math.round(shownFed) + ' / ' + curTarget + ' 人', W/2, 40); }
  // 返回大廳
...
function hudTap(p){
  if (p.y < CROWD_TOP){
    ...
    if (p.x > W-56){ muted = !muted; try{ localStorage.setItem('l2f-mute', muted?'1':'0'); }catch(e){} return true; }
  }
  return false;
}
...
function drawMenu(t){
  ...
  for (i=0;i<3;i++){
    ...
    ctx.fillText('同款連 ' + MODES[keys[i]].minChain + ' 顆・餵飽 ' + MODES[keys[i]].target + ' 人', W/2, y+60);
...
function startGame(forceLv){
  ...
  scene = 'play'; playing = true; startTime = Date.now();
    if (sprint){ banner = { text: '⏱ 限時衝刺!' + Math.round(SPRINT_SECS[modeKey]||80) + ' 秒內餵飽越多越好!', t: 3 }; }
...
function drawWin(t){
  ...
  ctx.fillText(sprint ? ('⏱ 時間到!共餵飽 ' + fed + ' 人') : '🎉 眾人都吃飽了!', W/2, 170);
...
function winTap(p){
  for (var i=0;i<winBtns.length;i++){
    var b = winBtns[i];
    if (p.x>b.x && p.x<b.x+b.w && p.y>b.y && p.y<b.y+b.h){
      if (b.act==='next'){ var nx = level+1; try{ localStorage.setItem('l2f-lvl-'+modeKey, ''+Math.max(maxLevel(), nx)); }catch(e){} startGame(nx); return; }
